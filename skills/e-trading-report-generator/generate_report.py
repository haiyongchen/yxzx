#!/usr/bin/env python3
"""
e交易专区收益成本统计报告生成器（Python版）
支持图表生成和新增指标（指标八：接入超半年无收益）
"""
...
def analyze_data(file_path):
    ...
    stats = {
        ...
        # 指标四更新：接入超过3个月，小于1年，总收益为0
        'indicator4': len(df[(df['确认接入时间'] > one_year_ago) & (df['确认接入时间'] <= three_months_ago) & (df['总收益情况'] == 0)]),
...
def create_chart1(risk_data, output_path):
    ...
    wedges, texts, autotexts = ax.pie(risk_data.values(), labels=risk_data.keys(), autopct='%1.1f%%', colors=colors, startangle=90, textprops={'fontsize': 12})
    for autotext in autotexts:
        autotext.set_color('white')
        autotext.set_fontweight('bold')
    ax.set_title('e交易专区风险等级分布', fontsize=16, fontweight='bold', pad=20)
    ax.legend(wedges, [f'{k}: {v}个' for k, v in risk_data.items()], title="风险等级", loc="center left", bbox_to_anchor=(1, 0, 0.5, 1))
    plt.tight_layout()
...
def create_chart2(revenue_dist, output_path):
    ...
    for bar in bars:
        ...
        ax.text(bar.get_x() + bar.get_width()/2., height, f'{int(height)}', ha='center', va='bottom', fontsize=11)
...
def create_word_report(stats, output_path, chart_paths):
    ...
    doc.add_paragraph(f'从上表可以看出，接入超1年，但长期零收益专区为0的有{stats["indicator1"]}个，长期低收益专区（总收益10w以下）共计{stats["indicator2"]}个，占比较高，因收益10w以下的专区过多，故进一步分析收益5w以下的专区，数量为{stats["indicator3"]}个；接入3个月至1年但无收益的专区有{stats["indicator4"]}个，需要重点关注；25年前接入但25年收益在5w以下的专区有{stats["indicator5"]}个，反映出早期接入专区的运营效率问题；特别值得关注的是，有{stats["indicator7"]}个专区在25年产生收益但26年暂无收益，存在客户流失风险；此外，接入超过半年但仍无收益的专区有{stats["indicator8"]}个，需要重点关注。')
    ...
    data_rows = [
        ...
        ['指标四', '接入超过3个月，小于1年，总收益为0', str(stats['indicator4']), '红色', '重点关注'],
...
def main():
    if len(sys.argv) < 2:
        print("用法: python generate_report.py <数据文件路径> [输出目录]")
        print("示例: python generate_report.py D:/data/专区数据.xlsx D:/output")
        sys.exit(1)
    
    data_file = sys.argv[1]
    output_dir = sys.argv[2] if len(sys.argv) > 2 else os.path.dirname(data_file)
    ...
    report_path = os.path.join(output_dir, 'e交易专区收益成本统计报告_生成.docx')
