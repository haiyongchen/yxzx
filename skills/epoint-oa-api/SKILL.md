...
```
D:\openclaw-workspace\skills\epoint-oa-api\scripts\oa_api.py
```

### 使用方法
```bash
python D:\openclaw-workspace\skills\epoint-oa-api\scripts\oa_api.py <API_PATH> "<JSON_PARAMS>"
```
...
执行指令：
```bash
python D:\openclaw-workspace\skills\epoint-oa-api\scripts\oa_api.py mail_getunreadlist_v7 "{\"currentpageindex\": 0, \"pagesize\": 10}"
```

### 更新 OA 接口库
获取所有分类及其接口详情并存入 `assets/`：
```bash
python D:\openclaw-workspace\skills\epoint-oa-api\scripts\update_api_docs.py
```

### 获取分类 (apiinnerservice)
执行指令：
```bash
python D:\openclaw-workspace\skills\epoint-oa-api\scripts\oa_api.py apiinnerservice/getApiShareCategory "{\"parentGuid\": \"a9a648b2-9122-4404-b2b7-b60e2f4f2562\"}"
```
