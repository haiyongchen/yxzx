...
for idx, mail in enumerate(mails):
    ...
    try:
        ...
        # 获取当前 URL（包含 detailguid）
        mail['link'] = page.url
        
        # 提取 detailguid
        if 'detailguid=' in page.url:
            mail['detailguid'] = page.url.split('detailguid=')[1].split('&')[0]
        else:
            mail['detailguid'] = ''
...
with open('D:/openclaw-workspace/oa_mails_with_links.json', 'w', encoding='utf-8') as f:
    json.dump(result, f, ensure_ascii=False, indent=2)

print(f"✅ 结果已保存：D:/openclaw-workspace/oa_mails_with_links.json")

# 输出表格数据
print("\n📊 表格数据预览:")
for mail in mails:
    print(f"{mail['index']} | {mail['subject'][:30]}... | {mail['link'][:60]}... | {mail['summary'][:50]}...")
