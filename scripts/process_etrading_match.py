...
def process_etrading_data():
    ...
    # 找到专区名称列（通常是第2列，索引1）
    zone_col = df_stats.columns[1]  # 专区名称
    print(f"   专区名称列: {zone_col}")
    print(f"   专区名称示例: {df_stats[zone_col].dropna().head(10).tolist()}")
    ...
    if file_2026_month:
        ...
        print(f"   平台名称列: {platform_col}")
        print(f"   平台名称示例: {df_month[platform_col].dropna().head(5).tolist()}")
        
        # 按平台名称分组汇总
        month_summary = df_month.groupby(platform_col).agg({
            revenue_col: 'sum',
            yoy_col: 'first',  # 取第一个同比值
            mom_col: 'first'   # 取第一个环比值
        })
    ...
    if file_2026_total:
        ...
        for idx, row in df_stats.iterrows():
            ...
            if zone_name in total_summary.index:
                df_stats.at[idx, 'e交易26年总收益_截至3月25日'] = total_summary.loc[zone_name, revenue_col]
                df_stats.at[idx, 'e交易总项目数'] = int(total_summary.loc[zone_name, project_col])
                match_count += 1
    ...
    if len(has_revenue) > 0:
        ...
        print(has_revenue[preview_cols].head(10).to_string())
    else:
        print("   暂无有收益数据的平台")
