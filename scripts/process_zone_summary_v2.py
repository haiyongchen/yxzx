...
def process_zone_summary():
    ...
    print(f"   合并后总行数: {len(df_summary)}")
    print(f"   列名: {df_summary.columns.tolist()}")
    
    # 找到专区名称列（通常是第2列，索引1）
    zone_col = df_summary.columns[1]  # 专区名称
    ...
    task_contract_col = df_task.columns[10]
    task_start_col = df_task.columns[5]
    task_cost_col = df_task.columns[8]
    
    # 找到合同号列和上线时间列
    contract_col = df_summary.columns[0]  # A列：合同号
    online_time_col = df_summary.columns[16]  # 专区实际上线时间
    
    print(f"   合同号列: {contract_col}")
    print(f"   上线时间列: {online_time_col}")
    ...
    # 统计
    print("\n9. 统计结果:")
    print(f"   有收益数据的平台数: {(df_summary['e交易总收益_总收益'] > 0).sum()}")
    print(f"   有总人工成本的平台数: {(df_summary['总人工成本'] > 0).sum()}")
    print(f"   有上线前人工成本的平台数: {(df_summary['上线前人工成本'] > 0).sum()}")
    
    # 保存结果
    output_file = base_dir / "专区信息汇总表_按省份分类_含收益和成本.xlsx"
