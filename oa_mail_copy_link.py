# -*- utf-8 -*-
"""
OA 邮件分析 - 点击 adv-copy-btn 获取真实邮件链接
"""
import sys
import os
import time
...
for idx, mail in enumerate(mails):
    ...
    try:
        ...
        # 查找并点击 adv-copy-btn 按钮
        try:
            copy_btn = page.locator('#adv-copy-btn').first
            if copy_btn.is_visible(timeout=3000):
                copy_btn.click(timeout=3000)
                time.sleep(1)
                
                # 从剪切板获取链接
                mail_link = pyperclip.paste()
                mail['link'] = mail_link
                print(f"    ✅ 链接：{mail_link[:120]}...")
            else:
                print(f"    ⚠️  按钮不可见")
                mail['link'] = page.url
...
with open('D:/openclaw-workspace/oa_mails_with_real_links.json', 'w', encoding='utf-8') as f:
    json.dump(result, f, ensure_ascii=False, indent=2)

print(f"✅ 结果已保存：D:/openclaw-workspace/oa_mails_with_real_links.json")
