...
for line in doc1_lines:
    if '|' in line and len(line) > 50:
        ...
        if len(cells) >= 10:
            ...
            # 检查是否是专区数据行（排除表头）
            if is_chen_province(province) and '分公司' not in cells[0] and '#ERROR!' not in cells[0] and '省份' not in cells[0]:
                doc1_zones.append({
                    "来源": "文档 1-专区管控表",
...
for line in doc2_lines:
    ...
    if in_25_new_table and '|' in line and len(line) > 50:
        ...
        # 检测表格结束
        if len(cells) > 0 and cells[0] in ['序号', '注', '专区推进', '低收益', '增量', '无量', '新开', '落地', '待接入', '公告', '湖北', '已上线', '收费标准', '专区管控', '落地部署']:
            if cells[0] == '序号':
                continue
            else:
                in_25_new_table = False
...
output_file = Path("D:/openclaw-workspace/output/陈海勇负责专区_12 省份_完整版.xlsx")
