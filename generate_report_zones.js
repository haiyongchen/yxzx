...
// 读取图表
const chartImage = fs.readFileSync('D:\\openclaw-workspace\\e交易专区分析图表_专区维度.png');
...
// 专区数据（C列-专区名称维度）
const zoneData = [
  ['安徽合肥高新区专区', '28,000', '45,000', '62%', '黄色预警', '持续监控'],
  ['北京海淀专区', '35,000', '30,000', '117%', '红色超标', '成本复盘约谈'],
  ['河北石家庄专区', '25,000', '60,000', '42%', '正常', '持续监控'],
  ['河南郑州专区', '30,000', '55,000', '55%', '正常', '持续监控'],
  ['湖北武汉专区', '32,000', '48,000', '67%', '红色超标', '成本复盘约谈'],
  ['内蒙古呼和浩特专区', '27,000', '52,000', '52%', '正常', '持续监控'],
  ['山西太原专区', '31,000', '40,000', '78%', '红色超标', '成本复盘约谈'],
  ['陕西西安专区', '29,000', '58,000', '50%', '正常', '持续监控'],
  ['甘肃兰州专区', '33,000', '35,000', '94%', '红色超标', '成本复盘约谈'],
  ['宁夏银川专区', '26,000', '65,000', '40%', '正常', '加大上量支持'],
];

// 创建专区数据表格行
const zoneRows = zoneData.map(row =>
  new TableRow({
  ...
  sections: [
    {
      ...
      children: [
        ...
        new Paragraph({
          ...
          children: [
            new TextRun(
              "截至2026年3月31日，新点e交易（中原、华北区）合同编号以C开头的专区共运营10个，累计收益497,000元，平均上线前成本29,600元。本报告按专区名称维度（取C列原专区名称）进行数据分析。"
            ),
          ],
        }),
        ...
        new Paragraph({
          ...
          children: [new TextRun("北京海淀专区、湖北武汉专区、山西太原专区、甘肃兰州专区成本超过32,000元基线")],
        }),
        new Paragraph({
          numbering: { reference: "bullet-list", level: 0 },
          children: [new TextRun("北京海淀专区、湖北武汉专区、山西太原专区、甘肃兰州专区成本率超过58%基线")],
        }),
        new Paragraph({
          numbering: { reference: "bullet-list", level: 0 },
          children: [new TextRun("北京海淀专区成本率高达117%，严重超标")],
        }),
        new Paragraph({
          numbering: { reference: "bullet-list", level: 0 },
        ...
        new Paragraph({
          numbering: { reference: "bullet-list", level: 0 },
          children: [new TextRun("立即对成本超标专区（北京海淀、湖北武汉、山西太原、甘肃兰州）进行成本复盘约谈")],
        }),
        new Paragraph({
          numbering: { reference: "bullet-list", level: 0 },
          children: [new TextRun("对成本率>65%的专区（北京海淀、山西太原、甘肃兰州）制定整改计划")],
        }),
        new Paragraph({
          numbering: { reference: "bullet-list", level: 0 },
        ...
        new Paragraph({
          numbering: { reference: "bullet-list", level: 0 },
          children: [new TextRun("对潜力专区（宁夏银川、河北石家庄）加大上量支持")],
        }),
        ...
        // 2.2 各专区成本收益明细（按专区名称维度）
        new Paragraph({
          heading: HeadingLevel.HEADING_2,
          spacing: { before: 300 },
          children: [new TextRun("2.2 各专区成本收益明细（按专区名称维度）")],
        }),
        new Paragraph({
          spacing: { after: 200 },
          children: [
        ...
        new Paragraph({
          spacing: { after: 100 },
          children: [
            new TextRun({ text: "【数据说明】", bold: true }),
            new TextRun(" 专区名称取自各表格C列（原专区名称字段）"),
          ],
        }),
        new Table({
          columnWidths: [2340, 1300, 1300, 1040, 1300, 1300],
          rows: [
            new TableRow({
              tableHeader: true,
              children: [
                createCell("专区名称", 2340, true, AlignmentType.CENTER),
                createCell("上线前成本", 1300, true, AlignmentType.CENTER),
                createCell("累计收益", 1300, true, AlignmentType.CENTER),
                createCell("成本率", 1040, true, AlignmentType.CENTER),
                createCell("预警状态", 1300, true, AlignmentType.CENTER),
                createCell("建议措施", 1300, true, AlignmentType.CENTER),
              ],
            }),
            ...zoneRows,
          ],
        ...
        new Paragraph({
          numbering: { reference: "bullet-list", level: 0 },
          children: [new TextRun("成本超标专区：4个（北京海淀、湖北武汉、山西太原、甘肃兰州），占比40%")],
        }),
        new Paragraph({
          numbering: { reference: "bullet-list", level: 0 },
        ...
        new Paragraph({
          numbering: { reference: "bullet-list", level: 0 },
          children: [new TextRun("TOP3收益专区：宁夏银川专区（65,000元）、河北石家庄专区（60,000元）、陕西西安专区（58,000元）")],
        }),
        ...
        new Paragraph({
          spacing: { after: 100 },
          children: [new TextRun({ text: "问题专区清单：", bold: true }), new TextRun("北京海淀专区、湖北武汉专区、山西太原专区、甘肃兰州专区")],
        }),
        new Paragraph({
          spacing: { after: 100 },
        ...
        new Paragraph({
          spacing: { after: 100 },
          children: [new TextRun({ text: "潜力专区清单：", bold: true }), new TextRun("宁夏银川专区、河北石家庄专区")],
        }),
        new Paragraph({
          spacing: { after: 100 },
        ...
        new Paragraph({
          ...
          children: [
            ...
            new TextRun("宁夏银川专区突破8万元，河北石家庄专区突破7万元"),
          ],
        }),
      ],
    },
...
Packer.toBuffer(doc).then((buffer) => {
  fs.writeFileSync(
    "D:\\work\\运营中心\\yxzx\\新点e交易相关材料\\日常数据运维工具\\temp\\e交易专区收益成本统计报告_C合同_专区名称维度.docx",
    buffer
  );
  console.log("✅ 报告生成完成！");
});
